          ...
          if (info.manager) {
                    try {
                      ...
                      for (const row of childRows) {
                                ...
                                console.warn(`Could not fetch child accounts for MCC ${id}:`, childErr.message);
                    }
          }
      } catch (infoErr) {
              // If we can't get info, still add it with basic details
          allAccounts.set(id, { id, name: `Account ${id}`, isManager: false, isTestAccount: false });
              console.warn(`Could not get info for account ${id}:`, infoErr.message);
      }
  }
