...
/**
 * Find the login-customer-id (MCC manager ID) needed to access a given customer.
 * For MCC sub-accounts, we need to set the manager's ID in the login-customer-id header.
 */
async function findLoginCustomerId(accessToken, developerToken, targetCustomerId) {
                const cleanTarget = targetCustomerId.replace(/-/g, '');
        ...
        // If the target is directly accessible, check if it's a manager or client
        // Try each accessible account to see if it's a manager that owns the target
        for (const id of directIds) {
                                if (id === cleanTarget) {
                                                                // Target is directly accessible - try without login-customer-id first
                                        // It might be a manager account querying itself
                                        continue;
                                }

                        try {
                                                        // Check if this accessible account is a manager
                                        const infoRows = await gaqlQuery(id, `
                                                                        SELECT customer.id, customer.manager
                                                                                                        FROM customer
                                                                                                                                        LIMIT 1
                                                                                                                                                                `, accessToken, developerToken);
                                        ...
                                        const info = infoRows[0]?.customer || {};
                                                        if (info.manager) {
                                                                                                // This is an MCC - check if target is a child
                                                                try {
                                                                                                                const childRows = await gaqlQuery(id, `
                                                                                                                                                                SELECT customer_client.client_customer
                                                                                                                                                                                                                FROM customer_client
                                                                                                                                                                                                                                                                WHERE customer_client.client_customer = 'customers/${cleanTarget}'
                                                                                                                                                                                                                                                                                                        `, accessToken, developerToken, id);

                                                                                                        if (childRows.length > 0) {
                                                                                                                                                                console.log(`[auth] Found MCC ${id} as manager for customer ${cleanTarget}`);
                                                                                                                                                                return id;
                                                                                                                }
                                                                        } catch (e) {
                                                                                                                // This MCC doesn't manage the target
                                                                }
                                                        }
                        } catch (e) {
                                                        // Skip accounts we can't query
                        }
        }

        // Target might be directly accessible without an MCC
        console.log(`[auth] No MCC found for customer ${cleanTarget}, using direct access`);
                return null;
}
...
export async function fetchAccountData(accessToken, customerId, periodType = 'mtd', loginCustomerId = null) {
                const devToken = process.env.GOOGLE_ADS_DEVELOPER_TOKEN;
                const periods = getPeriodDates(periodType);

        // If no loginCustomerId provided, try to find the MCC automatically
        let mccId = loginCustomerId;
                if (!mccId) {
                                        mccId = await findLoginCustomerId(accessToken, devToken, customerId);
                }
                console.log(`[fetchAccountData] Using loginCustomerId: ${mccId || 'none (direct access)'} for customer ${customerId}`);
...
export { getAccessibleCustomers, getPeriodDates, findLoginCustomerId };
